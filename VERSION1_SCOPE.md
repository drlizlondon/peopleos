...
### V1-16 — My Card and vCard export

Governed by POS-D049. PeopleOS cannot share the user's own details because no owner record exists. Deliver one owner record holding display name, optional phone numbers, email addresses, organisation and role; its Settings editing surface; the schema migration, backup/restore and iCloud replication contract; and vCard serialisation — the export deferred from V1-13. Do not add exchange transport, QR rendering, deep links, Apple Contacts writes, or any backend identity, account or handle.

**Independent acceptance:** the owner record is a singleton that can never appear in People, search, Today, Upcoming, Reach Out or the Relationship Engine, and cannot be archived, merged or duplicate-warned against; an absent or empty owner record is a valid state that blocks only sharing and never degrades another screen; serialisation emits a version 3.0 vCard that round-trips losslessly through the existing `parseVCard`, contains only conventional contact fields, and carries no relationship mode, cadence, follow-up, note, memory fact, Reach Out or interaction data; the migration is forward-only, backup and restore preserve the record, and an existing dataset without one upgrades cleanly; all existing tests and the performance ratchet remain green.

### V1-17 — Contact exchange

Governed by POS-D049. Deliver outbound sharing of the V1-16 owner card as a self-contained link — rendered as an on-screen QR code and offered through the iOS share sheet — plus the receiving route that parses the payload, presents the received person for review, and writes them to iPhone Contacts through the existing `createContact` boundary. Add the Associated Domains entitlement and the associated-domain file so the system Camera opens PeopleOS directly, and a no-app web fallback that renders the card and offers a vCard download. The exchange is one-directional in this package. Do not add camera permission, a bundled QR scanner, reciprocal or simultaneous exchange, presence, a PeopleOS-to-PeopleOS channel, any backend, or the intention prompt.

**Independent acceptance:** the whole payload lives in the URL fragment, so sharing and receiving work fully offline and no card content ever reaches a server or appears in a request path, query string or log; a received link resolves without PeopleOS holding camera permission; the exchange completes — contact saved to iPhone Contacts under the normal iOS permission flow, idempotent under the existing `operationId` retry contract — with **zero PeopleOS relationship decisions taken and zero PeopleOS records written**; a denied or restricted Contacts permission fails visibly without creating partial state; an absent owner record disables sharing with an explanatory state rather than an error; a malformed, truncated or foreign payload is rejected with a named reason and writes nothing; opening the link without the app installed reaches the web fallback; 390px layout, keyboard focus and accessible labels pass.

### V1-18 — Post-exchange intention

Governed by POS-D049. Deliver the prompt shown only after a V1-17 exchange completes, offering exactly **Reach out**, **Regular contact**, and dismiss. Reach out routes into the existing Reach Out capture; Regular contact initialises the existing regular-contact schedule; dismiss closes the prompt. Do not introduce a new intention concept, queue, status vocabulary, reminder type or schedule primitive, and do not add this prompt to manual Add Person, which is a PeopleOS-hosted flow whose intent is already expressed.

**Independent acceptance:** both actions reuse Reach Out (POS-D027/D028) and the regular-contact schedule (POS-D006) without a parallel record type; **dismiss leaves the iPhone contact intact and creates no PeopleOS Person, ContactMethod, FollowUp, FollowUpEvent, cadence or Reach Out entry**, and the same person can still be brought in later through the existing Import contacts path; the prompt is never reachable from manual capture; each action is idempotent under double-tap and rolls back atomically on failure; the prompt is fully dismissible by keyboard and screen reader, is never a blocking gate on the completed exchange, and 390px layout passes.
