...
## POS-D048 — The daily reminder is a ladder the user ends by opening PeopleOS

- **Status:** Accepted and implemented
- **Date:** 2026-08-16
- **Supersedes:** POS-D047's "no notification action buttons" clause and its one-occurrence-per-day plan shape. Everything else in POS-D047 stands: native local notifications only, Off by default, permission on explicit opt-in, no backend, no personal payload.

A single daily summary is missable, and a missed summary is the whole failure mode the notification exists to prevent. From the user's chosen time, PeopleOS now repeats the same private summary every three hours, and never places a reminder at or after 22:00 local time. The user's chosen time is always honoured: the cut-off bounds the reminders PeopleOS adds, not the time the user asked to be told. Each notification carries two actions — **View Today**, which opens the Today route, and **Not Now**, which does nothing at all.

There is still no overdue state and no second reminder system. Notifications remain pure downstream delivery of the same deterministic eligibility rules: they never create, complete, skip, or reschedule a Person, Interaction, FollowUp, FollowUpEvent, TodaySkip, Reach Out record, or Relationship Engine input. People leave Today only through the ordinary PeopleOS action.

The state model is two fields: which local date's cycle the user has already ended by opening PeopleOS after a notification was sent, and the instant the current plan was armed. The second exists because "an occurrence time has passed" is not the same as "a notification was delivered": occurrences earlier than the arming instant were never installed, and treating them as ignored silenced the whole first day whenever reminders were switched on after the chosen time. Turning reminders on or off, and choosing a new time, all re-arm from that moment. Both fields live in device-local storage — never the database, a backup, or iCloud — so that tapping View Today, which cold-launches the app, still counts as opening it. Dismissing, clearing, ignoring, and Not Now are all treated identically and leave the next reminder standing — which is precisely why the whole day's ladder is installed in advance instead of being scheduled in reaction to Not Now. iOS cannot run the Today engine when a local notification fires, and a non-foreground action cannot be relied on to wake the JavaScript layer, so a reactive design would silently drop reminders for the exact users who never open the app. Only opening PeopleOS ends a cycle, and the app is by definition running at that moment, so the remaining occurrences are cancelled during ordinary reconciliation — no delivery record, nothing in the database, backup, or sync.

Two costs are accepted. The bounded plan stays at 30 requests, so a ladder of five occurrences forecasts about six days instead of thirty; reopening PeopleOS replenishes it, and a user who never reopens is exactly the user the ladder is for. And the notification identifier now encodes an occurrence digit, so identifiers written by the single-notification release are still recognised — an upgraded install cancels the plan its previous version left behind instead of orphaning it.

The strongest alternative was scheduling the +3 hours only when Not Now is actually tapped. Rejected because the product rules give dismissing and ignoring the same consequence as Not Now, so the reactive design would have to fire without the tap anyway, and it would depend on background JavaScript execution iOS does not promise. This decision would be wrong if a future runtime could evaluate Today at delivery time; that runtime would let the cut-off and the ladder be computed live, and would need its own privacy decision first.

## POS-D049 — Contact exchange is a link over an owner record, not a file

- **Status:** Accepted; V1-16, V1-17 and V1-18 not started
- **Date:** 2026-08-16
- **Governs:** V1-16 (My Card and vCard export), V1-17 (Contact exchange), V1-18 (Post-exchange intention)

Exchanging contacts and deciding what a relationship becomes are two different acts, and PeopleOS has been conflating them. On the Add Person screen that conflation is correct and stays: the user opened PeopleOS and typed a name into it, so PeopleOS is the host and the Apple Contacts copy is the optional extra it is today. In an exchange the hosting inverts. Someone is standing in front of the user, the transaction is the contact itself, and PeopleOS has no right to make completing it conditional on answering a question about cadence. The exchange must therefore complete on its own — contact saved, done — and the PeopleOS intention prompt is a separate, fully dismissible layer after it. Dismissing writes nothing to PeopleOS: a People list that mirrors the address book is precisely what PeopleOS is not, and the existing Import contacts path already promotes someone later.

The transport is a self-contained link, rendered as a QR code and shareable through the iOS share sheet, with the entire payload in the URL fragment so nothing reaches a server. The link shape is chosen for what happens after the backend arrives: the same QR and the same route can later carry a PeopleOS identity that resolves remotely, while the fragment form keeps today's product fully offline and accountless. Because the link is an ordinary HTTPS URL under an associated domain, the system Camera opens it — PeopleOS never requests camera permission and ships no scanner — and a recipient without the app reaches a web fallback that renders the card and offers a vCard download.

The strongest alternative was AirDropping a vCard file, which is closer to how the idea was first described. Rejected because it is a dead end: it has no upgrade path to a PeopleOS-to-PeopleOS exchange, no fallback for a recipient without the app, and no way for PeopleOS to know the exchange happened. That last point is general and load-bearing. iOS gives third-party apps no notification that a contact was added, so if the exchange is performed by Apple — NameDrop, AirDrop, the Contacts app — PeopleOS cannot observe it and cannot offer the intention prompt at all. **The prompt is only reachable when PeopleOS itself performed the exchange.** Observing Apple's own exchanges would require broad Contacts read access and polling, which contradicts the narrow write-only boundary in `src/application/appleContacts.ts`.

Sharing requires knowing who the user is, and no owner record exists — there is no self, me, or profile entity in the schema. That record is the most expensive thing here to get wrong: it is a schema migration, it enters backup, restore and iCloud replication, and it is the seed of whatever identity the backend later issues. It is therefore built first and alone, in V1-16, before any transport depends on its shape.

Two limits are accepted rather than hidden. The exchange is one-directional: the user shares their card and the recipient receives it. True simultaneous exchange needs a live channel between two devices, and CloudKit as configured is deliberately private-database only — a peer channel built on it now would be a second architecture the backend immediately obsoletes. And peer-to-peer recognition — "Sarah wants to connect with you" — is explicitly **not** in these packages. It requires identity, discovery and a relay, all of which `VERSION1_SCOPE.md` excludes from Version 1. It is directional until the backend exists and needs its own decision record then.

This decision would be wrong if the backend arrives before V1-17 ships, in which case the fragment payload becomes a legacy path to support alongside a resolved identity rather than a stepping stone to it. That cost is bounded: the QR, the route and the owner record all survive, and only the payload encoding gains a second form.
