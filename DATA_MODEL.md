...
Settings evolve through package-owned forward migrations rather than pulling notification storage into Today. V1-10 adds `alreadyContactedDefaultReminderDays: 14`. The chargeable MVP adds `todaySummaryNotificationsEnabled: false` and `todaySummaryNotificationTime: "12:00"`; backups through schema 4 receive the same defaults. The final type above applies after migration, and each Settings mutation follows the repository revision/idempotency contract.
