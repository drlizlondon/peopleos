...
The application derives at most 30 one-off occurrences from the same fixed rules as Today: for each qualifying date, the user's chosen time and a reminder every three hours until 22:00 local. It safely installs and verifies a replacement before removing stale requests on launch, foreground/background transition, relationship-mode change, Settings change, or dataset revision. Current-local-date occurrences may include the current count; forecast occurrences use generic copy because local notification content is static while the app is closed. Which local date's reminder cycle the user has already ended by opening PeopleOS is device-session state only — it is never stored, backed up, or synced. Turning reminders Off cancels all PeopleOS summary IDs. Scheduling and tap handling do not increment the relationship dataset revision or mutate domain stores.
