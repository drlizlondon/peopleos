...
if (!allowDirty && status) {
  throw new Error("Release source must be committed and clean. Commit the accepted work before release preflight.");
}
let representedOnOriginMain = false;
try {
  git(["merge-base", "--is-ancestor", head, "origin/main"]);
  representedOnOriginMain = true;
} catch {
  // Kept false for the explicit release-source failure below.
}
if (!representedOnOriginMain) {
  throw new Error(`Release HEAD ${head.slice(0, 12)} is not represented on origin/main.`);
}
...
const distFiles = await filesBelow("dist-native");
if (distFiles.length === 0) throw new Error("The fresh native web build is empty.");
const nativeFiles = await filesBelow("ios/App/App/public");
const capacitorRuntimeFiles = ["cordova.js", "cordova_plugins.js"];
const copiedNativeFiles = nativeFiles.filter((file) => !capacitorRuntimeFiles.includes(file));
if (JSON.stringify(copiedNativeFiles) !== JSON.stringify(distFiles)) {
...
const compiledJavaScript = (await Promise.all(
  distFiles.filter((file) => file.endsWith(".js")).map((file) => text(join("dist-native", file)))
)).join("\n");
const expectedBuildCommit = `${head.slice(0, 12)}${status ? "-dirty" : ""}`;
if (!compiledJavaScript.includes(expectedBuildCommit)) {
  throw new Error(`Native build metadata does not identify ${expectedBuildCommit}. Rebuild with npm run native:sync.`);
}

console.log(`Release preflight passed for PeopleOS ${packageJson.version} (${buildNumbers[0]}).`);
console.log(`Verified ${distFiles.length} fresh native assets for commit ${expectedBuildCommit}.`);
