...
export default function TodayScreen({ activeMode = "personal", navigate, handoff = openContactHandoff }: TodayScreenProps) {
  ...
  return (
    <main className="screen today-screen" id="main-content" tabIndex={-1}>
      <header className="page-heading compact-heading today-heading today-heading-with-action">
        <div>
          <h2>Today</h2>
          <p>People you meant to contact.</p>
        </div>
        <button className="text-action" type="button" onClick={() => navigate("/upcoming")}>View upcoming</button>
      </header>
      {regularContactPrompt}
      {pageError && <div className="section-error"><p role="alert">{pageError}</p><button type="button" onClick={() => void load()}>Retry</button></div>}
      {projection.evaluationIssues.length > 0 && (
