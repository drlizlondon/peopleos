...
vi.mock("./contacts/capacitorAdapter", () => ({
  ...
  getIPhoneContactsAdapter: () => contactMocks.supported ? {
...
describe("selective iPhone Contacts journeys", () => {
  ...
  it("puts iPhone Contacts first and sends multiple picker contacts into the existing preview", async () => {
    contactMocks.pickContacts.mockResolvedValue({
      ...
      contacts: [
        ...
        {
          displayName: "Mum",
          phoneNumbers: [{ value: "+44 7900 123457", label: "Mobile" }],
          emailAddresses: [],
          organisation: "Family",
          jobTitle: "Organiser"
        }
    ...
    expect(document.querySelector(".import-summary-bar")).toHaveTextContent("2 selected from iPhone Contacts");
    ...
    expect(screen.getByRole("checkbox", { name: "Import Mum" })).toBeChecked();
  });
  ...
  it("treats picker cancellation as a no-op", async () => {
    contactMocks.pickContacts.mockResolvedValue({ status: "cancelled", contacts: [] });
    const user = userEvent.setup();
    render(<App />);

    await user.click(screen.getByRole("button", { name: "Choose from iPhone Contacts" }));
    await waitFor(() => expect(contactMocks.pickContacts).toHaveBeenCalledTimes(1));
  ...
  it("enriches the opened name-only Person from one selected iPhone contact instead of creating a duplicate", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Link iPhone contact" }));

    const review = await screen.findByRole("region", { name: "Link iPhone contact" });
    const reviewHeading = within(review).getByRole("heading", { name: "Link iPhone contact" });
  ...
  it("leaves the opened Person unchanged when profile contact selection is cancelled", async () => {
    ...
    expect(screen.queryByRole("region", { name: "Link iPhone contact" })).not.toBeInTheDocument();
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
    const data = await readAllData(db);
    expect(data.people).toHaveLength(1);
