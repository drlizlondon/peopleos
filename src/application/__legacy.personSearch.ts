/**
 * FROZEN COPY — the search implementation exactly as it stood at commit
 * `c8bcc86`, immediately before V1-R6 began optimising ranking.
 *
 * This exists solely so `personSearch.differential.test.ts` can prove that
 * making search faster did not change what it finds or the order it finds it
 * in. Ranking decides which People a user can locate, across eleven match
 * sources; a silent reordering produces a product that still looks like it
 * works, which is why this duplicate earns its keep for the duration of the
 * work.
 *
 * DO NOT import this from production code, and do not "fix" it — it is a
 * historical record, not a maintained module. V1-R6 ends by converting it to
 * golden fixtures and deleting it, alongside `__legacy.engine.ts`.
 */
...
export function legacyAssessmentsForSearch(
  data: PeopleOsData,
  clock: RelationshipClock
...
export function legacySearchPeopleFromData(
  data: PeopleOsData,
  ...
  const assessments = precomputedAssessments ?? legacyAssessmentsForSearch(data, options.clock);
...
export function legacyPersonFilterOptionsFromData(
  data: PeopleOsData,
  clock: RelationshipClock,
  precomputedAssessments?: PersonSearchAssessments
): PersonFilterOptions {
  ...
  const assessments = precomputedAssessments ?? legacyAssessmentsForSearch(data, clock);
