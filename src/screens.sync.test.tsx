...
vi.mock("./platformRouting", () => ({
  browserPathForLogicalPath: (path: string) => path,
  isNativeApplication: () => mocks.nativeApplication
}));
...
describe("Settings iCloud controls", () => {
  ...
  it("describes local-only browser storage truthfully", async () => {
    mocks.cloudSupported = false;
    render(<SettingsScreen navigate={vi.fn()} />);

    const section = (await screen.findByRole("heading", { name: "iCloud Sync" })).closest("section");
    expect(section).not.toBeNull();
    expect(within(section!).getByText("Stored in this browser only")).toBeInTheDocument();
    expect(within(section!).getByText(/stores this web app's data in this browser/i)).toBeInTheDocument();
