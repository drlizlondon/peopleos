...
Status: implementation contract, written before implementation on 28 July 2026.
...
The canonical schema is in `src/domain/schema.ts`; validation is in `src/domain/validation.ts`. `src/data/database.ts` now opens IndexedDB version 3: version 1 is the established local model, version 2 adds the CloudKit sync stores described below, and version 3 adds the independently reviewed relationship-mode migration. `src/data/repositories.ts` supplies generic optimistic-revision repositories, but compound application commands also write several stores directly in atomic IndexedDB transactions. Consequently sync capture cannot depend only on repository callbacks.
...
`ExternalIdentity` exists in the domain schema but has no current IndexedDB store or backup entry. It therefore cannot yet be produced by the application. The sync schema reserves `externalIdentities`; implementation adds it as a canonical empty-capable store and backup field so future records do not require a transport redesign. Existing backups migrate it to `[]`.

`metadata` is device-local and never synchronised. Sync metadata stores are also device-local.

## Durable local sync state

IndexedDB version 2 adds:
